...
export default function ContestantsAdminPage() {
  const { contestants, loading, fetchContestants, deleteContestant } = useContestants();
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingContestant, setEditingContestant] = useState<Contestant | undefined>(undefined);
  ...
  return (
    <div className="py-10 px-6 sm:px-8 max-w-5xl mx-auto space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Contestants</h1>
          <p className="text-muted-foreground mt-1">Manage the participants for your voting event.</p>
        </div>
