      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full border border-white/16 bg-white/14 backdrop-blur-md dark:border-white/12 dark:bg-white/10"
            />
          )}
  ...
  return (
    <motion.div
      animate={{
        ...
        borderRadius: visible ? "1.25rem" : "2rem",
        y: visible ? 20 : 0,
      }}
      transition={{
        type: "spring",
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-2xl border border-white/35 bg-white/75 px-4 py-8 shadow-[0_0_24px_rgba(34,42,53,0.06),0_1px_1px_rgba(0,0,0,0.05),0_0_0_1px_rgba(34,42,53,0.04),0_0_4px_rgba(34,42,53,0.08),0_16px_68px_rgba(47,48,55,0.05),0_1px_0_rgba(255,255,255,0.1)_inset] backdrop-blur-xl supports-backdrop-filter:bg-white/70 dark:border-white/15 dark:bg-neutral-900/75 dark:supports-backdrop-filter:bg-neutral-900/65",
            className,
          )}
        >
          {children}
